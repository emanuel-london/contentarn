  ...
  transformJson(obj) {
    ...
    if (typeof obj.data.type !== 'undefined' && obj.data.type == 'recipes') {
      ...
      const node = {
        ...
        category: (typeof item.relationships.category !== 'undefined') ?
         transformJsonGetCat(obj, item.relationships.category.data.id, "categories") : '',
      }
      nodes[0] = node;
    }
    else {
      ...
      const node = {
        ...
        category: (typeof item.relationships.category !== 'undefined') ?
         transformJsonGetCat(obj, item.relationships.category.data.id, "categories") : '',
        tags: (typeof item.relationships.tags !== 'undefined') ? transformJsonGetTags(obj, item.relationships.tags) : '',
      }
      nodes.push(node);
     });
    }
